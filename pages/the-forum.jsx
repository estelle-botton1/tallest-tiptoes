...
export default function TheForum() {
  ...
  useEffect(function () {
    client
      .fetch('*[_type == "forumPost"] | order(coalesce(sortOrder, 999) asc, _createdAt desc) { _id, title, format, date, preview, videoUrl, link, image, body }')
      .then(function (data) {
        setPosts(data);
        setLoading(false);
      })
      ...
      {featured && (
        <div style={{ padding: "24px 20px 0", cursor: "pointer" }} onClick={function () { setSelectedIndex(0); }}>
          ...
          <div style={{ padding: "14px 0" }}>
            ...
            {featured.preview && (
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "16px", color: c.muted, margin: 0, lineHeight: "1.5" }}>{featured.preview}</p>
            )}
          </div>
        </div>
      )}
...
function renderBlockText(block, i) {
  if (block._type === "block" && block.children) {
    var children = block.children.map(function (child, j) {
      ...
      if (child.marks && child.marks.length > 0 && block.markDefs) {
        var linkMark = null;
        child.marks.forEach(function (markKey) {
        ...
        if (linkMark) {
          return (
            <a key={j} href={linkMark.href} target="_blank" rel="noopener noreferrer" style={{ color: c.red, textDecoration: "underline", textUnderlineOffset: "3px" }}>{text}</a>
          );
        }
        ...
        if (isBold || isItalic) {
          return <span key={j} style={{ fontWeight: isBold ? "600" : "inherit", fontStyle: isItalic ? "italic" : "inherit" }}>{text}</span>;
        }
      }
    ...
    if (block.style === "h2") {
      return <h2 key={i} style={{ fontFamily: "'DM Serif Display', serif", fontSize: "24px", fontWeight: "400", margin: "28px 0 10px", color: c.black }}>{children}</h2>;
    }
    if (block.style === "h3") {
      return <h3 key={i} style={{ fontFamily: "'DM Serif Display', serif", fontSize: "20px", fontWeight: "400", margin: "24px 0 8px", color: c.black }}>{children}</h3>;
    }
    if (block.style === "blockquote") {
      return (
        <div key={i} style={{ padding: "20px", margin: "20px 0", borderLeft: "3px solid " + c.red, borderRight: "3px solid " + c.red, textAlign: "center" }}>
          <p style={{ fontFamily: "'DM Serif Display', serif", fontSize: "20px", fontStyle: "italic", color: c.ink, margin: 0, lineHeight: "1.4" }}>{children}</p>
        </div>
      );
    }
  ...
  if (block._type === "image" && block.asset) {
    return (
      <div key={i} style={{ margin: "24px 0", borderRadius: "3px", overflow: "hidden" }}>
    ...
    <div style={{ minHeight: "100vh", background: c.cream, fontFamily: "'Playfair Display', Georgia, serif", color: c.black }}>
      ...
      <div style={{ padding: "24px 20px" }}>
        ...
        {!post.body && post.preview && (
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "18px", color: c.ink, lineHeight: "1.7" }}>{post.preview}</p>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "0 20px 24px", gap: "16px" }}>
        <div style={{ flex: 1, height: "1px", background: c.pale }} />
        <div style={{ flex: 1, height: "1px", background: c.pale }} />
      </div>
      ...
      <div style={{ padding: "0 20px 32px" }}>
        {hasNext ? (
          <div onClick={onNext} style={{ textAlign: "center", padding: "16px", background: c.black, borderRadius: "3px", cursor: "pointer" }}>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.cream }}>NEXT ARTICLE</span>
          </div>
        ) : (
          <Link to="/" style={{ textDecoration: "none" }}>
            <div style={{ textAlign: "center", padding: "16px", background: c.black, borderRadius: "3px", cursor: "pointer" }}>
              <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.cream }}>RETURN TO HOME</span>
            </div>
          </Link>
        )}
      </div>
