...
var c = {
  red: "#A33B2E", oldRose: "#B98589", ink: "#3D3633", black: "#1A1A1A",
  cream: "#F6F0E8", parchment: "#EDE4D8", warm: "#E0D2C2", pale: "#EAE2D8",
  muted: "#8A7E72", light: "#B8AA9C", white: "#FBF8F4",
};
...
export default function TheShop() {
  ...
  useEffect(function () {
    ...
    if (productId && products.length > 0) {
      var match = products.find(function (p) { return p._id === productId; });
      if (match) setSelected(match);
    }
  }, [products, searchParams]);

  useEffect(function () {
    if (selected) { window.history.replaceState(null, "", "/the-shop?product=" + selected._id); }
    else { window.history.replaceState(null, "", "/the-shop"); }
  }, [selected]);

  var filtered = activeTab === "all" ? products : products.filter(function (p) { return p.category === activeTab; });

  if (selected) { return <ProductDetail product={selected} onClose={function () { setSelected(null); }} />; }
            ...
            return (
              <div key={product._id} onClick={function () { setSelected(product); }} style={{ cursor: "pointer" }}>
                <div style={{ aspectRatio: "4/5", maxHeight: "200px", overflow: "hidden", background: img ? "none" : "linear-gradient(160deg, " + c.warm + ", " + c.parchment + ")", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                  {img ? (<img src={urlFor(img).width(400).url()} alt={product.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />) : (<span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "9px", color: c.muted, opacity: 0.3 }}>PHOTO</span>)}
                  {product.status === "Sold" && (<div style={{ position: "absolute", top: "8px", right: "8px", padding: "3px 8px", background: c.black }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "9px", fontWeight: "600", letterSpacing: "1px", textTransform: "uppercase", color: c.cream }}>Sold</span></div>)}
                  {product.status === "Coming Soon" && (<div style={{ position: "absolute", top: "8px", right: "8px", padding: "3px 8px", background: c.red }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "9px", fontWeight: "600", letterSpacing: "1px", textTransform: "uppercase", color: c.cream }}>Soon</span></div>)}
                </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: c.cream, fontFamily: "'Playfair Display', Georgia, serif", color: c.black }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid " + c.pale }}>
        ...
        <span onClick={onClose} style={{ fontFamily: "'DM Serif Display', serif", fontSize: "16px", color: c.black, cursor: "pointer" }}>The Shop</span>
        <span onClick={function () {
          var url = "https://tallest-tiptoes.com/the-shop?product=" + product._id;
          if (navigator.share) { navigator.share({ title: product.title, url: url }); }
          else { navigator.clipboard.writeText(url); }
        }} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "13px", color: c.muted, cursor: "pointer" }}>Share</span>
      </div>
      <div onClick={function () { setZoomed(!zoomed); }} style={{ height: zoomed ? "auto" : "420px", maxHeight: zoomed ? "none" : "420px", overflow: zoomed ? "visible" : "hidden", background: img ? "none" : "linear-gradient(135deg, " + c.warm + ", " + c.parchment + ")", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", cursor: zoomed ? "zoom-out" : "zoom-in" }}>
        {img ? (<img src={urlFor(img).width(zoomed ? 1600 : 1000).url()} alt={product.title} style={{ width: "100%", height: zoomed ? "auto" : "100%", objectFit: zoomed ? "contain" : "cover" }} />) : null}
        {product.status === "Sold" && (<div style={{ position: "absolute", top: "16px", right: "16px", padding: "5px 14px", background: c.black }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "10px", fontWeight: "600", letterSpacing: "1.5px", textTransform: "uppercase", color: c.cream }}>Sold Out</span></div>)}
        {!zoomed && images.length > 1 && (<div style={{ position: "absolute", bottom: "10px", right: "10px", padding: "4px 10px", background: "rgba(0,0,0,0.5)", borderRadius: "2px" }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "10px", color: "#fff" }}>{activeImg + 1} / {images.length}</span></div>)}
      </div>
      {images.length > 1 && (<div style={{ display: "flex", gap: "6px", padding: "10px 20px", overflowX: "auto", scrollbarWidth: "none" }}>{images.map(function (img2, i) { return (<div key={i} onClick={function () { setActiveImg(i); setZoomed(false); }} style={{ width: "60px", height: "60px", flexShrink: 0, overflow: "hidden", border: i === activeImg ? "2px solid " + c.ink : "1px solid " + c.pale, cursor: "pointer" }}><img src={urlFor(img2).width(200).url()} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>); })}</div>)}
      ...
      {product.story && (<div style={{ padding: "20px" }}><div style={{ padding: "20px", background: c.parchment, borderRadius: "3px", borderLeft: "3px solid " + c.red }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", color: c.muted }}>THE STORY</span><p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "15px", fontStyle: "italic", color: c.ink, margin: "8px 0 0", lineHeight: "1.6" }}>{product.story}</p>{product.location && (<p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", color: c.muted, margin: "12px 0 0" }}>Found in {product.location}</p>)}</div></div>)}
      <div style={{ padding: "4px 20px 20px" }}>
        {product.status === "Available" && product.shopUrl && (<a href={product.shopUrl} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}><div style={{ textAlign: "center", padding: "15px", background: c.black, cursor: "pointer" }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.cream }}>BUY NOW</span></div></a>)}
        {product.status === "Available" && !product.shopUrl && (<div style={{ textAlign: "center", padding: "15px", background: c.black, opacity: 0.5 }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.cream }}>COMING SOON</span></div>)}
        {product.status === "Sold" && (<div style={{ textAlign: "center", padding: "15px", background: c.light }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.white }}>SOLD OUT</span></div>)}
        {product.status === "Coming Soon" && (<div style={{ textAlign: "center", padding: "15px", border: "1px solid " + c.black }}><span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", color: c.black }}>NOTIFY ME</span></div>)}
      </div>
      <HonestyBox />
